...
class DatasetRetrieveUpdateDestroyAPIView(RetrieveUpdateDestroyAPIView):
    permission_classes = (IsAuthenticated,)
    serializer_class = DatasetSerializer

    def retrieve(self, request, *args, **kwargs):
        try:
            dataset = Dataset.objects.get(pk=kwargs['pk'], user=request.user)
        except Exception:
...
class PhotoRemoveAPIView(DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            img = Image.objects.get(pk=int(kwargs['pk']), image_dataset__user_id=request.user.id)
        except Exception as e:
            raise NotFound('image not found')

        dataset_pk = img.image_dataset.first().id
        img.delete()
